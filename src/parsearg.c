...
void defaults(FILE *out, unsigned int *colonne, char shortoption, char *longoption, char *explain){
	...
	for(j = 0, k = 7; j < _strlen(longoption);j++, k++){
		if(k < colonne[0]){
	...
	for(j = 0, k = 0; j < _strlen(explain);j++, k++){
		if(k < colonne[1]){
...
void parser_usage(struct parser *parser){
	...
	_memset(&state, 0, sizeof(struct parser_state));
	...
	/*for(i = 0; options[i].longoption != 0; i++){
		if((c_ = _strlen(options[i].longoption)+5) > c)
		{	c = c_;
			if(options[i].args)
				c += _strlen(options[i].args);
		}
	}
	c++;*/
	/*n = 0;*/
	for(i = 0;z = 0, options[i].longoption != 0; i++){
		...
		for(j = 0, k = 7,z = 0; j < _strlen(options[i].longoption);j++, k++){
			if(k < colonne[0])
		...
		if(options[i].args){
			...
			fprintf(state.out_stream,"=");
			k++;
			for(k = k, j = 0, n = 0;j < _strlen(options[i].args)+((options[i].flags&OPTION_ARG_OPTIONAL) == OPTION_ARG_OPTIONAL); j++, k++, n++){
				if(k < colonne[0])
		...
		if(k != 0)
			for(k = k+z;k < colonne[0] + m +1; k++)
				fprintf(state.out_stream," "); 
		for(k = 0, j = 0; j < _strlen(options[i].explain); j++, k++){
			if(k < colonne[1])
				fprintf(state.out_stream,"%c", options[i].explain[j]);
			else{
				for(j = j; j < _strlen(options[i].explain) && options[i].explain[j] != ' '; j++)
					fprintf(state.out_stream,"%c",options[i].explain[j]);
				fprintf(state.out_stream,"\n");
...
void parser_short_usage(struct parser *parser){
	...
	for(i = 0, j = 0; options[i].longoption != 0; i++){
		if(options[i].flags&OPTION_ARG_OPTIONAL){
			upper(&k, 8+_strlen(options[i].args), l);
			if(printable(options[i].shortoption))
				fprintf(state.out_stream,"[-%c [%s]] ",options[i].shortoption, options[i].args);
			k += 8+_strlen(options[i].args);
		}
	}
	for(i = 0, j = 0; options[i].longoption != 0; i++){
		if((options[i].flags&OPTION_ARG_OPTIONAL) == 0 && options[i].args && options[i].shortoption){
			upper(&k, 6+_strlen(options[i].args),l);
			if(printable(options[i].shortoption))
				fprintf(state.out_stream,"[-%c %s] ",options[i].shortoption, options[i].args);
			k += 6+_strlen(options[i].args);
		}
	}
	for(i = 0;options[i].longoption != 0; i++){
		if(options[i].args == NULL){
			upper(&k, 5+_strlen(options[i].longoption), l);
			fprintf(state.out_stream,"[--%s] ", options[i].longoption);
			k += 5+_strlen(options[i].longoption);
		}
	}
	for(i = 0, j = 0; options[i].longoption != 0; i++){
		if(options[i].flags&OPTION_ARG_OPTIONAL){
			upper(&k, 8+_strlen(options[i].longoption) + _strlen(options[i].args), l);
			fprintf(state.out_stream,"[--%s[=%s]] ",options[i].longoption, options[i].args);
			k += 8+_strlen(options[i].longoption) + _strlen(options[i].args);
		}
	}
	for(i = 0, j = 0; options[i].longoption != 0; i++){
		if((options[i].flags&OPTION_ARG_OPTIONAL) == 0 && options[i].args){
			upper(&k, _strlen(options[i].longoption) + _strlen(options[i].args) +6, l);
			fprintf(state.out_stream,"[--%s=%s] ",options[i].longoption, options[i].args);
			k += _strlen(options[i].longoption) + _strlen(options[i].args) +6;
		}
	}
	upper(&k, 9, l);
...
void parser_parse(struct parser *parser, int argc, char **argv, /*unsigned int flags, unsigned int index,*/ void *input){
	...
	/*_memcpy(&p,parser, sizeof(struct parser));
	p.state = &state;*/
	for(state.arg_num = 1, len = 0, temporary = NULL; ok = 0, state.arg_num < (unsigned int)state.argc; state.arg_num++){
		...
		/*else	state.next = 0;*/
		if(_strlen(state.argv[state.arg_num]) > 1 && _strncmp(state.argv[state.arg_num],"--",2) == 0){
			if(end == 1){
				ok = 1;
				goto end;
			}
			if(_strlen(state.argv[state.arg_num]) == 2){
			...
			for(j = 0; ok == 0 && options[j].longoption != 0; j++){
				len = ((temporary = _strchr(&state.argv[state.arg_num][2], '='))) ? _strlen(temporary) : 0;
				if((ok = (_strncmp(options[j].longoption, &state.argv[state.arg_num][2],_strlen(&state.argv[state.arg_num][2]) - len) == 0 &&
					_strncmp(options[j].longoption, &state.argv[state.arg_num][2],_strlen(options[j].longoption)) == 0)))
				{	if(options[j].args){
			...
			if(ok == 0 && _strncmp(state.argv[state.arg_num],"--",_strlen(state.argv[state.arg_num])) != 0){
				if(_strcmp(state.argv[state.arg_num],"--help") == 0){
					parser_usage(state.parser);
					exit(EXIT_SUCCESS);
				}
				if(_strcmp(state.argv[state.arg_num],"--usage") == 0){
					parser_short_usage(state.parser);
					exit(EXIT_SUCCESS);
				}
				if(parser->program && parser->program->program_version && _strcmp(state.argv[state.arg_num],"--version") == 0){
			...
			if(*state.argv[state.arg_num] == '-' && _strlen(state.argv[state.arg_num]) >1){
						...
						if((ok = (state.argv[state.arg_num][j] == options[k].shortoption))){
							if(options[k].args){
								if((options[k].flags&OPTION_ARG_OPTIONAL) == OPTION_ARG_OPTIONAL){
									if(_strlen(&state.argv[state.arg_num][j]) > 1){
										parser->parse_opt(options[k].shortoption, &state.argv[state.arg_num][j+1], &state);
										j += _strlen(&state.argv[state.arg_num][j+1]);
									}else{
